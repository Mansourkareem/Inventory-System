...
# مسار نسبي للملف
file_path = os.path.join("Excel", "KSA.xlsx")  # افترض أن الملف في مجلد فرعي يسمى 'data'
...
# استخدام المسار النسبي للصورة
image_path = os.path.join("imeges",'mbuzz-light-logo.png')
...
# # خيار لتحميل الملف
# uploaded_file = st.file_uploader("قم بتحميل ملف Excel أو Google Sheets", type=["xlsx", "xls"])
...
#         # تحويل السيريال نمبر لنص إذا احتجنا
#         if 'Serial Number' in df.columns:
#             df['Serial Number'] = df['Serial Number'].astype(str)

#         # تخزين DataFrame في session_state
#         st.session_state.df = df
...
# else:
#     st.warning("يرجى تحميل ملف Excel.")

# # باقي كود المشروع (زر المخزون، الفرز، الإدخال، الخ)
# if 'show_stock' not in st.session_state:
#     st.session_state.show_stock = False
...
# # عرض المخزون إذا كانت الحالة True
# if st.session_state.show_stock and 'df' in st.session_state:
